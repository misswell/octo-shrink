...
| 渠道 | 最新版本 | 适合谁 | 获取方式 |
|------|---------|--------|---------|
| [GitHub Releases](https://github.com/misswell/octo-shrink/releases) | v2.5.35 | 想要最新版、Windows / Linux 用户 | 下载 DMG / 安装包，直发版支持在线更新 |
...
历史页每一行都换上了与「压缩完成」那一行对等的按钮 —— 另存为、对比查看、恢复原图 / 删除这次压缩结果、在访达中显示、复制日志，并且**只在这件事此刻真的做得到时才出现**：原图备份被清掉或压缩结果已被你删掉，对应按钮一起收起；「添加后缀」「输出到文件夹」两种模式的原图从没被覆盖过，所以给的是对等的「删除这次压缩结果」，而不是一个按下去什么也换不回来的「恢复原图」。清空历史现在会数清楚这一次带走几份原图备份，恢复完成时的提示也跟着输出方式走。三条产物线（直发 / App Store / Swift 原生）判据逐条一致。
