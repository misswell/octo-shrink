...
- ✅ 压缩设置面板**默认折叠**（三线一致）：两条 Tauri 线在 `index.html` 的 `#settingsPanel` 上写死 `collapsed`，Swift 线 `settingsExpanded = false`；只有用户点标题才切换，任何启动 / 导入 / 清空路径都不许自动展开（详见测试 `tests/settings-collapse.cjs`）
- ✅ 两条产物线功能对齐：JXL 已从前端输出格式下拉移除（两版一致）；GIF 两版均有压缩功能（Direct gifsicle 减色更优，App Store image crate 重编码，属质量差异非功能差异）
- ✅ 对比视图已改为独立原生窗口（未发版）：`compare.html` + `compare_window.js`，label="compare"，由 `open_compare_window` 命令创建（Direct 用 tauri:// 内嵌页，App Store 用本地 HTTP 页），自由缩放可大于主窗口、自带红绿灯；载荷经 `pending_compare` 状态 + `take_compare_window_payload` 首屏取回，`compare-open` / `compare-results-changed` 事件双向同步；`on_window_event` 仅在 label=="main" 关闭时 exit(0)；首屏与 resize 均按"适合窗口"fit 适配（先 showPanel 再测量视口，隐藏态测量会得到 0 而回退 100%，大图只显示局部），缩放下限 0.02，重置按钮=重新适配，用户手动缩放后 resize 不再打断；**分割线只有一个入口：在图片上拖动**（跟着鼠标走，平移图片时不抢手势）—— 底部那根 `<input type=range id="compareRange">` 白线已删除（它和拖拽是同一件事的两种入口，视觉上只是一条压住图片边缘的线），分割状态改由 `compareSplitPercent` 承载，`tests/compare-slider.cjs` 守这条
- ✅ 安全暂停 / 继续 / 停止已接入（三线一致；**v2.5.39 起「队列」与「执行会话」正式分家**）：会话四态 `idle`/`running`/`paused`/`stopping` 只管"这一轮在不在跑"，队列项六态 `pending`/`running`/`done`/`failed`/`removed`/`restored` 才管"这个文件还需不需要处理"。暂停与停止都只拦「还没开始」的文件，绝不 kill 正在跑的 CLI 子进程；**停止结束的是这一轮**——没轮到的文件仍是 `pending`，主按钮变成「继续压缩」，新一轮只提交 pending（`deferred` 事件 + `AcquireOutcome::Stopped`，绝不写成取消）。进度是**队列的派生值**（`getQueueProgress` / `QueueProgress`），停止继续都不会清零；`failed` 只有显式「重试」才回到 `pending`。**取消一个文件只 `wake_waiters()`、绝不 `resume()`**，整批取消走一次 `cancel_batch`（详见「安全暂停 / 继续 / 停止」）
...
| 安全暂停 / 继续 / 停止 | `CompressionScheduler`（`pause_compression` / `resume_compression` / `stop_compression` / `get_compression_state` / `cancel_file` / `cancel_batch` / `clear_cancel_queue`）：闸门只拦「还没开始」的文件，正在跑的 CLI 子进程绝不 kill；`Notify` + 250 ms 超时轮询，等待者用 `acquire` 自带退出条件（**判断顺序：取消 → 停止 → 暂停/名额**），三种结局用 `AcquireOutcome` 分开回给调用方；`compress_files` / `compress_smart` 收 `sessionId` 并逐条进度事件带回（`ProgressStatus`：`queued`/`starting`/`completed`/`failed`/`cancelled`/**`deferred`**）；阶段变化经 `compression-state-change` 事件播报 | 同上（进程内引擎同样只在新任务起点等待）| 两版行为一致；Swift 线为 `CompressionScheduler.acquireOutcome(cancelled:)` / `AppState.stopBatch()`，队列进度走 `QueueProgress` 派生。**取消只 `wake_waiters()`，绝不 `resume()`**；**停止只结束这一轮，剩下的文件仍 `pending`** |
| 对比窗口（compare 独立窗口）| WebviewUrl::App 加载内嵌 compare.html，经 convertFileSrc / read_image_dataurl 读图 | WebviewUrl::External 指向本地 HTTP 服务器 `http://localhost:<port>/compare.html`（`frontend_http_port()`），经 read_image_dataurl（bookmark 授权范围内）读图，窗口与主窗口同一模式：始终可见 + 创建时设主题背景色（compare.html 首帧自设同步主题背景，不露白） | 沙盒阻止 tauri://；两版窗口行为一致，URL 按 feature 分叉。⚠️ 不许用 `visible(false)` + on_page_load 延迟 show——`webview.show()` 经 wry 只作用于 WKWebView 视图、NSWindow 仍隐藏，沙盒版对比窗口永远弹不出来（v2.5.44 实测并已修） |
...
| 状态 | 回答什么问题 | 取值 | 存在哪 |
|---|---|---|---|
| **队列状态** `QueueStatus` / 前端 `queueItems[].state` | 「这个文件还需不需要处理」 | `pending` / `running` / `done` / `failed` / `removed` / `restored` | 队列项本身（前端 `queueItems`，Swift `AppState.items`） |
| **会话状态** `CompressionState` / `CompressionPhase` | 「当前这一轮在不在跑」 | `idle` / `running` / `paused` / `stopping` | 调度器（Rust `CompressionScheduler`，Swift 同） |

- **暂停和停止都不写队列状态**：暂停中等待的文件仍是 `pending`（只是渲染成「已暂停」），停止后没轮到的文件仍是 `pending`（照旧渲染「等待中」）。`cancelled` **不再是**持久状态，取消 = `removed`（移出队列）。
- **一轮 = 一次执行会话**：`compress_files` / `compress_smart` 带 `sessionId`（`CompressionSessionResult` 原样回传），**每条进度事件都带 `sessionId`**；前端 `executionSession` 用它挡住上一轮迟到的事件（光靠 `queueRevision` 挡不住"同一队列里的两轮"）。
- **进度属于队列，不属于某一轮**：前端 `getQueueProgress()` / Swift `QueueProgress` 从队列项现算（`total = pending+running+done+failed`，`processed = done+failed`），所以停止后继续是从 40/100 接着长，绝不是 0/60；**失败也算"已处理"**，否则 98 成功 + 2 失败的队列永远停在 98%。

**会话的四个阶段**：`idle` / `running` / `paused` / `stopping`。值域是**唯一**的真相来源，前端不许再从"批次还没结束"猜状态：
...
进度事件的 `ProgressStatus`（Rust `commands.rs::ProgressStatus`，camelCase 序列化）与前端的分支一一对应：
`queued`（排上了）/ `starting`（真开工，写 `running`）/ `completed`、`failed`（带 `result`）/ `cancelled`（用户移出队列 → 前端写 `removed`）/ **`deferred`（停止没轮到 → 前端保持 `pending`）**。
`deferred` **不进 `results`、不计入 `sessionProcessed`** —— 它压根没执行；给它编一个"失败"结果就是把没发生的事写进账本。
...
- **「继续」有两种，别再合成一个**：暂停中的「继续」= `resume_compression`（**同一个会话**继续，绝不新建批次、绝不重发 `compress_files`）；已经停止回到 idle 的「继续压缩」= 主按钮 `startCompression`，**新开一轮只提交 `pending`**（`getPendingQueuePaths()` / Swift `items.filter { $0.status == .pending }`）。已 `done` 的绝不重压，`failed` 的要用户显式点「重试」。
- **闸门里的判断顺序不可改：取消 → 停止 → 暂停/名额**（Rust `CompressionScheduler::acquire` / Swift `acquireOutcome(cancelled:)`）：
  - 取消排最前，是"闸门关着却没人开"的正解 —— 等待者自己带退出条件，被取消的文件直接结束并上报 `cancelled`，永不启动，而不是靠取消操作偷偷开门；
  - **停止排在暂停之前**，否则"暂停中按停止"的等待者会一直堵在关着的闸门上，直到用户点「继续」——那正是「停止」的反面。
- **`AcquireOutcome` 三态必须分开回给调用方**（Rust `enum AcquireOutcome { Acquired(ParallelPermit), Cancelled, Stopped }` ↔ Swift `CompressionScheduler.AcquireOutcome`）：老实现用一个 `Option`/`Permit?` 的 `None` 同时表示"这个文件被取消"和"整批被停止"，调用方只能统一报 `cancelled` —— 那就是"停止后 70 张图全变成已跳过"的根源。拿到 `Stopped` 的 worker **什么都不做**（队列项保持 `pending`），拿到 `Cancelled` 的才写 `removed`。
- **停止是会话级的、且不可逆**：`stop_compression`（Rust，无需传路径）/ `AppState.stopBatch()`（Swift）只做三件事 —— 置 `stopping`、唤醒等待者、`pendingAutoCompress = false`。闸门自己认得 `stopping`，排在后面的文件一个个自行退出并收到 `deferred`，所以不存在"部分已停、部分还在排队"的中间态。停止后即使有人误调 `resume()` 也不放行。
- **停止之后不许自动续跑**：`pendingAutoCompress` 在按下停止那一刻就清掉，且一轮收尾时 `stoppedByUser` 为真就跳过自动续队列。用户刚说"停下"，再被"自动压缩"拉起来就是没听他说话（`tests/queue-progress.cjs` 有断言）。
- **状态在界面上要看得出来**：
  - 进度按钮：`running` → 转圈 +「压缩中…」/「转换中…」；`paused` → **静态暂停图标** +「暂停中…」（转圈动画必须停下 —— 动画还在转就是用户判断"到底暂停没有"的依据）；`stopping` → 转圈 +「正在停止…」（那几个文件真的还在跑）。
  - 队列行：等待中的行在暂停时显示暂停图标 +「已暂停」，**停止后照旧显示「等待中」**（它真的还在队列里等下一轮）；真正在跑的行照旧转圈，暂停 / 停止期间写「收尾中…」—— 它确实还在跑，画成停下来就是撒谎。
  - 摘要：`N / M 已处理`（`M` 是**整个队列**，不是这一轮的目标）+ `· 失败 N` + `· 已暂停` / `· 正在停止` + CPU 行，**不许**写"还有几个在跑"的数字 —— 那个数字在事件之后就没有下一个事件来更新它了。
  - 按钮：`running` → [暂停][停止]；`paused` → [继续][停止]；`stopping` → 两个都禁用+「正在停止…」；`idle` 时主按钮看**队列**：还有 `pending` 且已处理过 → 「继续压缩」，还有 `pending` 没处理过 → 「开始压缩」，全处理完 → 「压缩完成」，有失败 → 「处理完成 · N 个失败」。**绝不是** finally 里一律写「压缩完成」（那会把"这一轮停了"说成"队列干完了"）。
  - 进度条填充 = `processed / total`（真实比例，`frontend/style.css` 的 `.compress-btn-fill` 不许再写死 `width:100%` 靠脉冲动画假装有进度）；暂停 / 停止时动画停下、宽度停在真实比例上。
...
- 回归：`tests/pause-resume.cjs`（四态切换、暂停停动画、停止不可逆、失败回滚、单写入口、spinner 不散落）、`tests/queue-progress.cjs`（追加导入不打断当前轮、累计进度、停止后不续跑）、**`tests/compression-session.cjs`（Start→Pause→Resume 同一轮 / Start→Stop→Continue 换轮 / Pause→Stop→Continue / 旧事件隔离 / 失败不自动重试 / 进度不倒扣）**、`commands.rs` 的 `stopping_the_batch_lets_running_jobs_finish_and_closes_the_gate_for_good` / `stopping_while_paused_finishes_the_batch_instead_of_waiting_for_resume` / `stopping_does_not_leak_into_the_next_batch` / `stopping_defers_a_waiting_file_while_removing_it_cancels_it` / `the_progress_status_wire_format_is_what_the_frontend_matches` / `the_wire_format_is_the_four_state_strings_the_frontend_knows` ↔ `scripts/test_swift_history.sh` 第 [27]、[28]、**[29]** 组。
...
- tests/pause-resume.cjs（`npm run test:frontend`）— 压缩会话状态机：四态切换与按钮/文案映射、暂停时 loading 必须停（在跑的行写「收尾中…」）、停止不可逆且不自动续跑、IPC 失败回滚、主按钮按队列派生（继续压缩 / 完成 / 处理完成·N 失败）、**isCompressing / compressionPaused 只许由 setCompressionState 写**（grep 源码）
- tests/queue-progress.cjs（`npm run test:frontend`）— 队列生命周期：追加导入不打断当前执行会话、进度按队列累计、停止后不复跑
- **tests/compression-session.cjs（`npm run test:frontend`）— 执行会话模型：Start→Pause→Resume 是同一轮（绝不再发一次 invoke）、Start→Stop→Continue 才新开一轮且只带 pending、被停止延后的项必须仍是 pending（绝不变成取消）、旧会话的迟到事件被 `sessionId` 拦住、failed 不被继续压缩自动重试（显式重试后已处理数不倒退）、进度绝不清零**
